...
const PadelCourt = () => {
    ...
    const saveToCloud = async () => {
        ...
        try {
            ...
            if (drillId) {
                ...
                const { data, error } = await supabase.from('drills').insert({ ...payload, user_id: currentUser.id }).select().single();
                if (error) throw error;
    ...
    const fixBoundary = (pos: any, radius: number) => {
        ...
        if (y < radius) y = radius; if (y > height - radius) y = height - radius;
        return { x, y };
    };
    ...
    const handleDragEnd = (e: any, id: string) => {
        ...
        setPlayers(prev => prev.map(p => p.id === id ? { ...p, x: newPos.x, y: newPos.y } : p));
    };
    ...
    const undoLast = () => { if(isOwner) setLines(lines.slice(0, -1)); };
    ...
    if (isLoading) return <div className="text-white text-center py-20 animate-pulse">A carregar...</div>;
            ...
            <div className="flex flex-col lg:flex-row gap-8 py-8 w-full max-w-6xl mx-auto items-start px-4">
                ...
                <div className="flex flex-col items-center gap-6 w-full lg:w-auto flex-shrink-0">
                    {isOwner && (
                        <div className="flex flex-col gap-3 w-full max-w-[360px]">
                            ...
                            <div className="flex gap-2 p-2 bg-slate-800 rounded-xl shadow border border-slate-700 justify-center">
                                ...
                                <button onClick={clearBoard} className="flex-1 text-red-400 hover:text-red-300 hover:bg-red-500/10 py-1 rounded transition flex justify-center"><Trash2 size={20} /></button>
                                <button onClick={downloadImage} className="flex-1 text-green-400 hover:text-green-300 hover:bg-green-500/10 py-1 rounded transition flex justify-center"><Download size={20} /></button>
                            </div>
                        </div>
                    )}
                    ...
                    <Stage
                        ...
                        className={`bg-white shadow-2xl rounded-xl overflow-hidden touch-none border-[6px] border-slate-800 ring-1 ring-slate-700 ${isOwner ? 'cursor-crosshair' : 'cursor-default opacity-90'}`}
                    ...
                    >
                        <Layer>
                            ...
                            <Rect x={0} y={0} width={width} height={height} stroke="white" strokeWidth={4} listening={false} />
                            ...
                            <Line points={[width / 2, 160, width / 2, height / 2]} stroke="white" strokeWidth={2} listening={false} />
                            ...
                            {players.map((p) => (
                                <Circle
                                    key={p.id} x={p.x} y={p.y} radius={p.id === 'ball' ? 8 : 18} fill={p.color} stroke="white" strokeWidth={p.id === 'ball' ? 1 : 2}
                                    draggable={isOwner}
                                    dragBoundFunc={(pos) => fixBoundary(pos, p.id === 'ball' ? 8 : 18)} onDragEnd={(e) => handleDragEnd(e, p.id)}
                                />
                            ))}
                        </Layer>
                    </Stage>
                    ...
                    <div className="bg-slate-800 p-8 rounded-3xl border border-slate-700 shadow-2xl h-full relative overflow-hidden">
                        ...
                        <div className="flex items-center justify-between mb-8 pb-4 border-b border-slate-700">
                            <div className="flex items-center gap-3">
                                <div className={`p-2 rounded-lg ${isOwner ? 'bg-green-500/10 text-green-500' : 'bg-blue-500/10 text-blue-400'}`}>
                                    {isOwner ? <FileText size={24} /> : <Globe size={24} />}
                                </div>
                                <div>
                                    <h2 className="text-xl font-bold text-white">{isOwner ? 'Detalhes' : 'Modo Leitura'}</h2>
                                    {!isOwner && <p className="text-xs text-blue-400 font-bold">A visualizar tática da comunidade</p>}
                                </div>
                            </div>
                            {isOwner && (
                                <button onClick={() => setIsPublic(!isPublic)} className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider transition border ${isPublic ? 'bg-blue-500/20 text-blue-400 border-blue-500/50' : 'bg-slate-700 text-slate-400 border-slate-600'}`}>
                                    {isPublic ? <Globe size={14} /> : <Lock size={14} />} {isPublic ? 'Público' : 'Privado'}
                                </button>
                            )}
                        </div>
