            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
                    <div>
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <Link href="/dashboard/perfil"><button className="p-2 bg-slate-800 hover:bg-slate-700 rounded-full border border-slate-700 text-slate-400 hover:text-white transition"><User size={18} /></button></Link>
                            {isAdmin && <Link href="/admin"><button className="flex items-center gap-2 px-3 py-1.5 bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 text-red-400 rounded-full text-xs font-bold uppercase tracking-wider transition"><Shield size={14} /> Backoffice</button></Link>}
                        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                        ...
                        <Link href="/dashboard/alunos">
                            <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 flex items-center justify-between hover:border-blue-500/50 hover:shadow-lg hover:shadow-blue-500/10 transition group cursor-pointer h-full">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div><p className="text-slate-400 text-xs uppercase font-bold tracking-wider">Meus Alunos</p><p className="text-3xl font-black text-white">{stats.students}</p></div>
                                </div>
                        ...
                        <Link href="/dashboard/planos">
                            <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 flex items-center justify-between hover:border-purple-500/50 hover:shadow-lg hover:shadow-purple-500/10 transition group cursor-pointer h-full">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div><p className="text-slate-400 text-xs uppercase font-bold tracking-wider">Planos de Aula</p><p className="text-3xl font-black text-white">{stats.plans}</p></div>
                                </div>
                    ...
                    <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 flex items-center justify-between hover:border-green-500/50 hover:shadow-lg hover:shadow-green-500/10 transition group h-full">
                        <div className="flex items-center gap-4">
                            ...
                            <div><p className="text-slate-400 text-xs uppercase font-bold tracking-wider">Biblioteca</p><p className="text-3xl font-black text-white">{drills.length}</p></div>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-slate-800 p-4 rounded-2xl border border-slate-700 mb-8 space-y-4 sticky top-4 z-30 shadow-2xl shadow-black/50">
                    <div className="relative">
                        ...
                        <input type="text" placeholder="Pesquisar..." className="w-full bg-slate-900 border border-slate-700 rounded-lg py-2.5 pl-10 text-white focus:outline-none focus:border-green-500 transition" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                    </div>
                    <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                        {CATEGORIES.map(cat => (
                            <button key={cat} onClick={() => setSelectedCategory(cat)} className={`whitespace-nowrap px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider border transition ${selectedCategory === cat ? 'bg-green-500 text-slate-900 border-green-500' : 'bg-slate-900 text-slate-400 border-slate-700 hover:border-slate-500'}`}>{cat}</button>
                        ))}
                    </div>
                </div>
                            ...
                            <Link href={`/dashboard/tatica?id=${drill.id}`} key={drill.id}>
                                <div className="bg-slate-800 rounded-xl overflow-hidden border border-slate-700 hover:border-green-500/50 transition group flex flex-col hover:shadow-2xl hover:shadow-black/50 hover:-translate-y-1 h-full">
                                    <div className="relative h-52 w-full bg-slate-900 border-b border-slate-700 group-hover:opacity-90 transition p-2">
                                        {drill.image_url ? <img src={drill.image_url} alt={drill.title} className="w-full h-full object-contain" /> : <div className="w-full h-full flex flex-col items-center justify-center gap-2 opacity-20"><LayoutDashboard size={40} className="text-slate-400" /><span className="text-xs text-slate-500">Sem pré-visualização</span></div>}
                                        <div className="absolute top-3 left-3"><span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider border backdrop-blur-md shadow-lg ${getCategoryColor(drill.category || 'Geral')}`}>{drill.category || 'Geral'}</span></div>
                                    </div>
                                    <div className="p-4 flex-1 flex flex-col">
                                        <div className="flex justify-between items-start gap-2 mb-2">
                                            ...
                                            <button onClick={(e) => deleteDrill(e, drill.id)} className="text-slate-600 hover:text-red-400 transition p-1 hover:bg-slate-700 rounded" title="Apagar"><Trash2 size={16} /></button>
                                        </div>
