...
import {
    Trash2, Plus, Calendar, Search, Filter, User, Shield, Users, Layers, LayoutDashboard, Lock, ArrowRight, Loader2,
} from "lucide-react";
    ...
    useEffect(() => {
        ...
        const getData = async () => {
            try {
                ...
                // Trocámos getUser() por getSession().
                // O getUser() estava a falhar (AuthSessionMissing) porque é muito estrito.
                // O getSession() usa a sessão local que já tens ativa.
                const { data: { session }, error: authError } = await supabase.auth.getSession();

                if (!isMounted) return;

                if (authError || !session?.user) {
                    console.log("Sessão perdida, redirecionando...");
                ...
                // Carregar Perfil (Agora deve funcionar sem travar)
                ...
                if (isMounted && profile) {
                    // Forçar a leitura correta dos booleanos
                    const adminStatus = profile.is_admin === true || String(profile.is_admin) === 'true';
                    const coachStatus = profile.role === 'coach' || String(profile.role) === 'coach';
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
                    <div>
                        ...
                        <p className="text-slate-400">
                            Tens <span className="text-white font-bold">{drills.length}</span> exercícios na tua biblioteca.
                        </p>
                    </div>
